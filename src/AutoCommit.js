...
class AutoCommit {
  ...
  async svnAutoCommit(callback) {
    try {
      ...
      if (this.comment) {
        ...
        execSync(`svn commit -m "feat: ${this.comment}"`, {
          encoding: 'utf-8',
          stdio: 'inherit',
        });
      ...
      process.nextTick(async () => {
        ...
        execSync(`svn commit -m "feat: ${comment}"`, {
          encoding: 'utf-8',
          stdio: 'inherit',
        });
  ...
  svnAddIgnoreExec() {
    execSync('svn propset svn:ignore -F svnignore.txt .', {
      encoding: 'utf-8',
      stdio: 'inherit',
    });

    execSync('svn propset svn:ignore "coverage" tests/unit', {
      encoding: 'utf-8',
      stdio: 'inherit',
    });
  ...
  svnAddIgnored(callback) {
    ...
    // 全局
    const addedGlobal = execSync('svn propget svn:global-ignores .', {
      encoding: 'utf-8',
    });

    // 本地
    if (!addedGlobal) {
      execSync('svn propset svn:global-ignores "node_modules .happypack .git" .', {
        encoding: 'utf-8',
        stdio: 'inherit',
      });
    }

    const added = execSync('svn propget svn:ignore .', {
      encoding: 'utf-8',
    });
      ...
      this.svnAddIgnored(() => {
        if (this.isMissingAll(status)) {
          ...
          statusArr.forEach((path) => {
            path = path.trim();
            if (path) execSync(`svn delete ${path}`);
          });
        }
